...
for (const g of gls) {
  ...
  if(d>0||c>0||o!==0) ;(() => {})(g.coa.code, '|', g.coa.name.substring(0,25), '| O='+o.toLocaleString('id-ID'), '| D='+d.toLocaleString('id-ID'), '| K='+c.toLocaleString('id-ID'), '| CL='+cl.toLocaleString('id-ID'));
}
;(() => {})('\nGRAND TOTAL: O='+totO.toLocaleString('id-ID'), 'D='+totD.toLocaleString('id-ID'), 'K='+totC.toLocaleString('id-ID'));
;(() => {})('IsBalanced (D==K)?', Math.abs(totD-totC) < 0.01 ? 'YES' : 'NO (diff='+(totD-totC).toLocaleString('id-ID')+')');
