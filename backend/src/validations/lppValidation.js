...
const decimalAllowNegativeSchema = z.number().refine((val) => {
  const decimalPart = val.toString().split(".")[1];
  return !decimalPart || decimalPart.length <= 2;
}, "Maksimal 2 digit desimal");
...
const detailSchema = z.object({
  ...
  nomorBukti: z
    ...
    .optional()
    .or(z.literal("")),
  jenisPembayaran: z.enum(
    ["CASH", "TRANSFER", "DEBIT", "CREDIT_CARD", "QRIS"],
    {
...
export const batchUpdateDetailsValidation = z
  .object({
    create: z.array(createLppDetailValidation).optional().default([]),
    update: z
      .array(
        z
          .object({
            ...
            tanggalTransaksi: z.coerce
              ...
              .refine((val) => val <= new Date(), {
                message:
                  "Tanggal transaksi tidak boleh melebihi tanggal sekarang",
              })
              .optional(),
            keterangan: z
              .string()
          ...
          .refine((data) => Object.keys(data).length > 1, {
            // minimal id + satu field lain
            message:
              "Minimal satu field selain ID harus diisi untuk update detail",
          })
      )
      .optional()
      .default([]),
    delete: z.array(uuidSchema).optional().default([]),
  })
  .refine(
    (data) =>
...
export const createLppValidation = z.object({
  ...
  sisaUangDikembalikan: decimalAllowNegativeSchema,
  keterangan: z
    .string()
    .max(500, "Keterangan maksimal 500 karakter")
    .optional()
...
export { uuidSchema, decimalSchema, nonNegativeDecimalSchema, detailSchema };
