...
function getBasePath`
  ...
  if (!code.includes('name="payrollConfigId"')) {
    code = code.replace(
      /<SelectItem value="HARIAN_BULANAN">2\. Harian di Bayar Bulanan<\/SelectItem>[\s\S]*?<\/SelectContent>\s*<\/Select>\s*<FormMessage \/><\/FormItem>\s*\)} \/>/,
  ...
  // 6. parse submission
  if (filePath.includes('updateFormData') && !code.includes("key === 'payrollConfigId'")) {
    code = code.replace(
      'Object.entries(values).forEach(([key, value]) => {',
      `Object.entries(values).forEach(([key, value]) => {
            if (key === 'payrollConfigId' && value === 'none') value = null;`
    );
  } else if (!filePath.includes('updateFormData') && !code.includes("key === 'payrollConfigId'")) {
    code = code.replace(
      'Object.entries(values).forEach(([key, value]) => {',
      `Object.entries(values).forEach(([key, value]) => {
  ...
  // Finally carefully inject useEffect logic
  let lines = code.split('\\n');
  if (!code.includes('getConfigs')) {
    let newLines = [];
    for (let i = 0; i < lines.length; i++) {
        newLines.push(lines[i]);
        if (lines[i].includes('getLocations();') && lines[i].includes('        ')) {
            newLines.push('        const getConfigs = async () => {');
            newLines.push('            const res = await fetchPayrollConfigs();');
            newLines.push('            if (res.data && !res.error) setPayrollConfigs(res.data);');
            newLines.push('        };');
            newLines.push('        getConfigs();');
        }
    }
    code = newLines.join('\\n');
  }
