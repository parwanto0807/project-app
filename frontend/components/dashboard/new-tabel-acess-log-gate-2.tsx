...
export default function NewAccessLogTableGate2() {
  ...
  const fetchAccessLogs = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:3000/api/access-log/getAccessLogsGate2?`);
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    socket.on("new-access-log-gate2", handleNewAccessLog);

    return () => {
      socket.off("new-access-log-gate2", handleNewAccessLog);
    };
  }, [socket]);
