...
import { Banknote, RefreshCw, Info, Upload, X, FileText, AlertCircle } from "lucide-react";
...
const EditOpexDialog: React.FC<Props> = ({ open, onOpenChange, onSuccess, data }) => {
    ...
    const [previewUrl, setPreviewUrl] = useState<string | null>(data.receiptUrl || null);

    const [formData, setFormData] = useState({
        date: data.date ? new Date(data.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        description: data.description,
        amount: data.amount.toString(),
        expenseAccountId: data.expenseAccountId || (data.expenseAccount as any)?.id || "",
        paidFromAccountId: data.paidFromAccountId || (data.paidFromAccount as any)?.id || ""
    });

    useEffect(() => {
        if (!open) return;

        // Reset form to latest data prop when opening
        setFormData({
            date: data.date ? new Date(data.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
            description: data.description,
            amount: data.amount.toString(),
            expenseAccountId: data.expenseAccountId || "", // Use ID directly if available
            paidFromAccountId: data.paidFromAccountId || ""
        });
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
        if (data.receiptUrl) {
            if (data.receiptUrl.startsWith('http') || data.receiptUrl.startsWith('blob:')) {
                setPreviewUrl(data.receiptUrl);
            } else {
                // Ensure leading slash
                const path = data.receiptUrl.startsWith('/') ? data.receiptUrl : `/${data.receiptUrl}`;
                setPreviewUrl(`${apiUrl}${path}`);
            }
        } else {
            setPreviewUrl(null);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error("Terjadi kesalahan koneksi atau sistem saat menyimpan perubahan");
        } finally {
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] rounded-3xl p-0 overflow-hidden border-none shadow-2xl max-h-[90vh] overflow-y-auto">
                <form onSubmit={handleSubmit}>
                    <div className="bg-gradient-to-br from-amber-500 to-orange-600 p-6 text-white sticky top-0 z-10">
                        <DialogHeader>
                            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
                                <Banknote className="h-6 w-6" />
                                Edit Biaya Operasional
                            </DialogTitle>
                            <DialogDescription className="text-amber-100 opacity-90">
                                Perbarui rincian pengeluaran.
                            </DialogDescription>
                        </DialogHeader>
                    </div>
                    ...
                    <div className="p-6 space-y-6 bg-white">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <Label htmlFor="edit-date" className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                                    Tanggal Biaya <span className="text-rose-500">*</span>
                                </Label>
                                <Input
                                    id="edit-date"
                                    type="date"
                                    className="rounded-xl border-slate-200 focus:ring-amber-500 h-11 transition-all hover:border-amber-300"
                                    value={formData.date}
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <Label htmlFor="edit-amount" className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                                ...
                                <Input
                                    ...
                                    type="number"
                                    placeholder="0"
                                    className="rounded-xl border-slate-200 focus:ring-amber-500 h-11 font-bold transition-all hover:border-amber-300"
                        ...
                        <div className="space-y-2">
                            <Label htmlFor="edit-description" className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                                Keterangan / Deskripsi <span className="text-rose-500">*</span>
                            </Label>
                            <Textarea
                                id="edit-description"
                                placeholder="Contoh: Pembayaran listrik kantor bulan Januari 2024"
                                className="rounded-xl border-slate-200 focus:ring-amber-500 min-h-[80px] transition-all hover:border-amber-300"
                        ...
                        <div className="space-y-3">
                            ...
                            <div
                                ...
                                className={cn(
                                    ...
                                    selectedFile || previewUrl ? "border-amber-200 bg-amber-50/30" : "border-slate-200 hover:border-amber-400 hover:bg-amber-50/30"
                                )}
                            >
                                <input
                                    type="file"
                                ...
                                {previewUrl || selectedFile ? (
                                    <div className="flex flex-col items-center w-full animate-in fade-in zoom-in duration-300">
                                        {previewUrl && !previewUrl.endsWith('.pdf') ? (
                                            <div className="relative w-full max-w-[200px] aspect-video rounded-lg overflow-hidden border border-amber-200 shadow-sm mb-3">
                                        ...
                                        <p className="text-xs font-medium text-amber-600">Klik untuk mengganti file</p>
                                    </div>
                                ) : (
                                    <>
                                        <div className="p-3 bg-slate-100 rounded-2xl text-slate-400 group-hover:bg-amber-100 group-hover:text-amber-500 transition-colors">
                                            <Upload className="h-6 w-6" />
                                        </div>
                                        <div className="text-center">
                                            <p className="text-sm font-bold text-slate-600 group-hover:text-amber-600">Pilih atau Drag Bukti Nota</p>
                        ...
                        <div className="bg-amber-50 p-4 rounded-2xl flex gap-3 border border-amber-100">
                            ...
                            <div className="space-y-1">
                                ...
                                <p className="text-[10px] text-amber-700 leading-relaxed italic">
                                    "Perubahan data akan disimpan. Pastikan data sudah benar sebelum melakukan Posting (Approval)."
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <DialogFooter className="p-6 bg-slate-50 border-t border-slate-100 gap-2 sticky bottom-0">
                        ...
                        <Button
                            type="submit"
                            disabled={submitting}
                            className="bg-amber-600 hover:bg-amber-700 text-white rounded-xl px-8 font-bold shadow-lg shadow-amber-100 h-11 transition-all hover:-translate-y-0.5"
                        >
                            {submitting ? (
                                <>
