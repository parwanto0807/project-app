...
// --- FUNGSI BARU: CONVERT IMAGE KE BASE64 (SOLUSI GAMBAR KOSONG) ---
// Fungsi ini menggunakan axios untuk mendownload gambar sebagai buffer lalu convert ke Base64
// Ini jauh lebih stabil daripada Canvas untuk PDF Generator
const convertImageToBase64 = async (url: string): Promise<string> => {
    try {
        ...
        console.error(`Gagal memuat gambar PDF: ${url}`, error);
        return "/placeholder.png"; // Fallback image jika gagal
    }
};

// --- 4. DOKUMEN PDF (LAYOUT ASLI ANDA) ---
export function BAPPdfDocument({ bap }: { bap: BAPData }) {
    // Ubah path.resolve menjadi string biasa agar tidak error di client-side
    const logoPath = '/LogoMd.png'; 
    
    const workItems = bap.salesOrder.items && bap.salesOrder.items.length > 0
        ? bap.salesOrder.items
        : [{ id: '1', name: 'Pekerjaan Utama', description: bap.workDescription || 'Pekerjaan sesuai dengan spesifikasi yang telah disepakati', productId: 'default', qty: 1, price: 0, total: 0, uom: 'Paket' }];
                            ...
                            {photoChunk.map((photo, index) => (
                                <View key={photo.id || `${pageIndex}-${index}`} style={styles.photoItem}>
                                    <View style={styles.photoImageContainer}>
                                        {/* INI KUNCINYA: photo.photoUrl di sini sudah berupa Base64 string dari tombol download */}
...
// --- 5. KOMPONEN TOMBOL DOWNLOAD (WRAPPER) ---
// Gunakan komponen ini di halaman utama Anda, bukan BAPPdfDocument langsung
export function BAPDownloadButton({ bap }: { bap: BAPData }) {
    ...
    const preparePdf = async () => {
        ...
        try {
            ...
            if (newBap.photos && newBap.photos.length > 0) {
                ...
                const processedPhotos = await Promise.all(
                    newBap.photos.map(async (photo: BAPPhoto) => {
                        ...
                        return {
                            ...photo,
                            photoUrl: base64 // Ganti URL dengan Base64 agar PDF bisa baca
                        };
                    })
                );
            ...
            alert("Gagal memproses gambar. Coba refresh.");
        } finally {
