...
import { RABCreateInput, CostType, CategoryRAB } from "@/types/rab";
...
interface RABFormProps {
    projects: Project[];
    products: Product[];
    onSubmit: (data: Omit<RABCreateInput, 'createdById'>) => void;
    isSubmitting: boolean;
    error?: string | null;
    user: { id: string } | null;
}

export function RABForm({ projects, products, onSubmit, isSubmitting, error, user }: RABFormProps) {
    // State untuk form
    const [formData, setFormData] = useState<Omit<RABCreateInput, 'createdById'>>({
        projectId: "",
        name: "",
        description: "",
        rabDetails: [],
    ...
    const calculateTotals = () => {
        return formData.rabDetails.reduce((acc, detail) => {
    ...
    // Di dalam RABForm component, perbaiki handleSubmit:
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Basic validation
        if (!formData.projectId || !formData.name || formData.rabDetails.length === 0) {
        ...
        try {
            // Gabungkan form data dengan user.id
            const submitData: RABCreateInput = {
                ...formData,
                createdById: user.id, // Gunakan user.id
                rabDetails: formData.rabDetails.map(detail => ({
                    ...detail,
                    qty: Number(detail.qty),
                    price: Number(detail.price),
                    categoryRab: detail.categoryRab as CategoryRAB,
                    costType: detail.costType as CostType,
        ...
        <div className="w-full mx-auto p-4 bg-white dark:bg-slate-900 dark:text-white rounded-lg shadow-sm">
            ...
            <div className="border-b border-gray-200 pb-4 mb-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h1 className="text-2xl font-bold">Create New RAB</h1>
                </div>
                <p className="text-gray-600">Fill in the details below to create a new RAB from Projects</p>
            </div>
            <form onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    ...
                    <Card className="bg-gradient-to-r from-white to-blue-50 dark:from-slate-800 dark:to-slate-900">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Enter the basic details for this RAB
                            </CardDescription>
                        </CardHeader>
                    ...
                    <Card className="bg-gradient-to-r from-white to-blue-50 dark:from-slate-800 dark:to-slate-900">
                        ...
                        <CardContent className="space-y-2">
                            <div className="flex justify-between">
                                ...
                                <span className="font-semibold">{formData.rabDetails.length}</span>
                            </div>
                            <div className="flex justify-between">
                                <span>Subtotal:</span>
                ...
                <Card className="bg-gradient-to-r from-white to-blue-50 dark:from-slate-800 dark:to-slate-900">
                    <CardHeader className="flex flex-row items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                Add items to your RAB
                            </CardDescription>
                        </div>
                    </CardHeader>
                    <CardContent>
                        {formData.rabDetails.length === 0 ? (
                ...
                <div className="flex justify-end gap-4 mt-6">
                    ...
                    <Button
                        type="submit"
                        disabled={isSubmitting || formData.rabDetails.length === 0}
                        className="flex items-center gap-2"
                    >
                        {isSubmitting ? "Creating..." : "Create RAB"}
                    </Button>
                </div>
            </form>
        </div>
