...
export async function uploadFotoAfterCreate(
    ...
    ;((...args: any[]) => {})("🔍 [DEBUG] uploadFotoAfterCreate called", {
        createdDetails,
        fotoBuktiMap,
        lppId,
    });
...
const CreateLppFormInput: React.FC<CreateLppFormInputProps> = ({
    ...
    const pullFromPrDetails = React.useCallback(() => {
        ...
        if (prItems.length > 0) {
            ;((...args: any[]) => {})(`⭐ [LPP-PULL] Pulling ${prItems.length} items directly from PR details`);
            replace(prItems);
            setFotoBuktiMap({});
    ...
    const handlePullData = React.useCallback(async () => {
        ...
        if (!searchNomorPr) {
            ;((...args: any[]) => {})("⚠️ [LPP-PULL] nomorPr is empty, pulling from PR details");
            pullFromPrDetails();
            return;
        }

        try {
            ;((...args: any[]) => {})(`🔍 [LPP-PULL] Searching PO for: ${searchNomorPr}`);
            ...
            ;((...args: any[]) => {})(`📊 [LPP-PULL] Found ${pos?.length || 0} PO candidates`);
            ...
            if (pos && pos.length > 0) {
                ...
                for (const poShort of pos) {
                    ;((...args: any[]) => {})(`📦 [LPP-PULL] Checking PO: ${poShort.poNumber} (ID: ${poShort.id})`);

                    const fullPo = await getPurchaseOrderById(poShort.id);
                    ;((...args: any[]) => {})(`📄 [LPP-PULL] Full PO loaded, Executions: ${fullPo.PurchaseExecution?.length || 0}`);
                ...
                if (allReceiptItems.length > 0) {
                    ...
                    ;((...args: any[]) => {})("ℹ️ [LPP-PULL] No executed receipts found in these POs. Falling back to PR details.");
                    pullFromPrDetails();
                }
            } else {
                ;((...args: any[]) => {})("ℹ️ [LPP-PULL] No PO found matching this PR number. Falling back to PR details.");
                pullFromPrDetails();
            }
        } catch (error) {
    ...
    const onSubmitForm = async (data: FormValues) => {
        ;((...args: any[]) => {})("📋 Raw Form Data (before mapping):", data);
        ...
        try {
            ...
            ;((...args: any[]) => {})("📤 Final LPP Data:", lppData);
