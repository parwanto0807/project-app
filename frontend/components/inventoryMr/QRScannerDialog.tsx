...
export const QRScannerDialog: React.FC<QRScannerDialogProps> = ({
    ...
    const stopCamera = React.useCallback(() => {
        ...
        if (streamRef.current) {
            try {
                streamRef.current.getTracks().forEach(track => {
                    ;;((...args: any[]) => {})("Stopping track:", track.kind)
                    track.stop()
                })
            } catch (err) {
        ...
        ;;((...args: any[]) => {})("Camera stopped")
    }, [])

    const startCamera = async () => {
        try {
            setError(null)
            ;;((...args: any[]) => {})("Starting camera...")
            ...
            if (videoRef.current) {
                ...
                ;;((...args: any[]) => {})("Camera started successfully")
            }
        } catch (err) {
    ...
    const scanQRCode = React.useCallback(() => {
        ...
        if (video.readyState === video.HAVE_ENOUGH_DATA) {
            ...
            if (code) {
                ;;((...args: any[]) => {})("QR Code detected:", code.data)
    ...
    React.useEffect(() => {
        ...
        return () => {
            ;;((...args: any[]) => {})("Component unmounting, cleaning up...")
            stopCamera()
        }
    }, [open, stopCamera])
