...
export default function StockOpnameForm({
    ...
    const [priceDialog, setPriceDialog] = useState<{ open: boolean; index: number } | null>(null);
    const [priceRef, setPriceRef] = useState<any>(null);
            ...
            if (warehouseId) {
                try {
                    const [stockResponse, priceResponse] = await Promise.all([
                        api.get('/api/inventory/latest-stock', {
                            params: { productId, warehouseId }
                        }),
                        api.get('/api/inventory/price-reference', {
                            params: { productId, warehouseId }
                        })
                    ]);

                    if (stockResponse.data.success) {
                        const latestStock = stockResponse.data.data;
                    ...
                    // Auto-collect harga satuan dari acuan (harga terakhir tercatat)
                    if (priceResponse.data.success && priceResponse.data.data?.masterPrice) {
                        form.setValue(`items.${index}.hargaSatuan`, Number(priceResponse.data.data.masterPrice.price) || 0);
                    }
                } catch (error) {
                    console.error("Failed to fetch latest stock/price:", error);
                    toast.error("Gagal mengambil stok/harga terbaru");
    ...
    const openPriceDialog = async (index: number) => {
        ...
        const warehouseId = form.getValues("warehouseId");
        setPriceRef(null);
        setPriceDialog({ open: true, index });
        setPriceLoading(true);
        try {
            const response = await api.get('/api/inventory/price-reference', {
                params: { productId, warehouseId: warehouseId || undefined }
            });
            setPriceRef(response.data.data);
        } catch (error) {
            console.error("Failed to fetch price reference:", error);
            toast.error("Gagal mengambil acuan harga");
        } finally {
            setPriceLoading(false);
        }
    };

    const applyPrice = (index: number, price: number) => {
        form.setValue(`items.${index}.hargaSatuan`, Number(price) || 0);
        setPriceDialog(null);
        toast.success("Harga satuan diterapkan", {
            description: formatCurrency(Number(price) || 0)
        });
    };
    ...
    return (
        <div className="w-full mx-auto px-6 space-y-8 animate-in fade-in duration-500">
                                                        ...
                                                        >
                                                            ...
                                                            <TableCell className="align-top p-2">
                                                                ...
                                                                <Button
                                                                    ...
                                                                    disabled={!form.watch(`items.${index}.productId`)}
                                                                    title="Cek acuan harga dari PO, Pembelian, dan Master Product"
                                                                >
                                                                    <Tag className="h-3.5 w-3.5 mr-1" /> Cek Harga
                                                                </Button>
                                                            </TableCell>
                                                            <TableCell className="align-top p-2 text-center">
                                                                <div className="flex flex-col gap-1 py-2">
            ...
            <Dialog
                ...
                onOpenChange={(open) => { if (!open) setPriceDialog(null); }}
            >
                <DialogContent className="sm:max-w-3xl max-h-[85vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>Cek Harga Satuan</DialogTitle>
                        <DialogDescription>
                            {priceRef?.product
                                ? `${priceRef.product.code} - ${priceRef.product.name}`
                                : "Memuat acuan harga dari Purchase Order, Pembelian, dan Master Product..."}
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-6">
                            ...
                            <section>
                                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2 flex items-center gap-2">
                                    <Package className="h-4 w-4 text-blue-600 dark:text-blue-400" /> Master Product
                                </h4>
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm dark:text-slate-300">
                                    <div>Satuan Beli: <b>{priceRef.product.purchaseUnit || "-"}</b></div>
                                    <div>Satuan Simpan: <b>{priceRef.product.storageUnit || "-"}</b></div>
                                    <div>Satuan Pakai: <b>{priceRef.product.usageUnit || "-"}</b></div>
                                    <div>Kategori: <b>{priceRef.product.category || "-"}</b></div>
                                </div>
                                {priceRef.masterPrice ? (
                                    <div className="mt-3 flex items-center justify-between gap-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3">
                                        <div>
                                            <div className="text-xs text-green-700 dark:text-green-400 font-medium">Harga Terakhir Tercatat</div>
                                            <div className="text-lg font-bold text-green-800 dark:text-green-300">{formatCurrency(priceRef.masterPrice.price)}</div>
                                            <div className="text-xs text-green-600 dark:text-green-500">
                                                {priceRef.masterPrice.referenceNo || "-"} • {format(new Date(priceRef.masterPrice.date), "dd MMM yyyy")}
                                                {priceRef.masterPrice.warehouse ? ` • ${priceRef.masterPrice.warehouse}` : ""}
                                            </div>
                                        </div>
                                        <Button size="sm" onClick={() => applyPrice(priceDialog!.index, priceRef.masterPrice.price)}>
                                            <Check className="h-4 w-4 mr-1" /> Pakai
                                        </Button>
                                    </div>
                                ) : (
                                    <div className="mt-3 text-sm text-amber-600 dark:text-amber-400">
                                        Tidak ada riwayat harga untuk produk ini.
                                    </div>
                                )}
                                {priceRef.avgLastPrice > 0 && (
                                    <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                                        Rata-rata {priceRef.priceHistory.length} transaksi terakhir: <b>{formatCurrency(priceRef.avgLastPrice)}</b>
                                    </div>
                                )}
                            </section>
                            ...
                            <section>
                                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">
                                    Acuan Harga Purchase Order (PO)
                                </h4>
                                {priceRef.poPrices.length === 0 ? (
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Tidak ada data PO untuk produk ini.</p>
                                ) : (
                                    <div className="space-y-2">
                                        {priceRef.poPrices.map((po: any) => (
                                            <div key={po.id} className="flex items-center justify-between gap-4 border rounded-lg p-3 dark:border-slate-700">
                                                <div>
                                                    <div className="font-medium text-sm dark:text-slate-200">{po.poNumber}</div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400">
                                                        {po.supplier || "-"} • {format(new Date(po.orderDate), "dd MMM yyyy")} • {po.status}
                                                    </div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400">
                                                        Qty: {formatNumber(po.quantity)} {priceRef.product.storageUnit}
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">{formatCurrency(po.unitPrice)}</div>
                                                    {po.unitPriceActual > 0 && (
                                                        <div className="text-xs text-green-600 dark:text-green-400">Aktual: {formatCurrency(po.unitPriceActual)}</div>
                                                    )}
                                                    <Button
                                                        size="sm"
                                                        variant="outline"
                                                        className="mt-1"
                                                        onClick={() => applyPrice(priceDialog!.index, po.unitPriceActual > 0 ? po.unitPriceActual : po.unitPrice)}
                                                    >
                                                        <Check className="h-3.5 w-3.5 mr-1" /> Pakai
                                                    </Button>
                                                </div>
                                            </div>
                                        ))}
                            ...
                            {/* Acuan Harga GR / Penerimaan */}
                            <section>
                                <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-2">
                                    Acuan Harga Penerimaan Barang (GR)
                                </h4>
                                {priceRef.grPrices.length === 0 ? (
                                    <p className="text-sm text-slate-500 dark:text-slate-400">Tidak ada data penerimaan barang untuk produk ini.</p>
                                ) : (
                                    <div className="space-y-2">
                                        {priceRef.grPrices.map((gr: any) => (
                                            <div key={gr.id} className="flex items-center justify-between gap-4 border rounded-lg p-3 dark:border-slate-700">
                                                <div>
                                                    <div className="font-medium text-sm dark:text-slate-200">{gr.grNumber}</div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400">
                                                        {gr.supplier || "-"} • {gr.receivedDate ? format(new Date(gr.receivedDate), "dd MMM yyyy") : "-"}
                                                        {gr.poNumber ? ` • ${gr.poNumber}` : ""}
                                                    </div>
                                                    <div className="text-xs text-slate-500 dark:text-slate-400">
                                                        Qty: {formatNumber(gr.qtyReceived)} {priceRef.product.storageUnit}
                                                    </div>
                                                </div>
                                                <div className="text-right">
                                                    <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">{formatCurrency(gr.unitPrice)}</div>
                                                    <Button
                                                        size="sm"
                                                        variant="outline"
                                                        className="mt-1"
                                                        onClick={() => applyPrice(priceDialog!.index, gr.unitPrice)}
                                                    >
                                                        <Check className="h-3.5 w-3.5 mr-1" /> Pakai
                                                    </Button>
                                                </div>
