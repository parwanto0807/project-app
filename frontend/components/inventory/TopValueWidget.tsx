...
import { DollarSign, Printer, Package, ArrowRight, Loader2 } from 'lucide-react';
import { getTopValue, TopValueItem } from '@/lib/action/inventory/inventoryAction';
import { format } from 'date-fns';
import { id as idLocale } from 'date-fns/locale';
import { toast } from 'sonner';
import TopValuePdf from './TopValuePdf';
import { Badge } from '@/components/ui/badge';

interface TopValueWidgetProps {
  period: string;
  warehouseId?: string;
  warehouseName?: string;
}

export default function TopValueWidget({ period, warehouseId, warehouseName = 'All Warehouses' }: TopValueWidgetProps) {
  const [top5, setTop5] = useState<TopValueItem[]>([]);
  const [top20, setTop20] = useState<TopValueItem[]>([]);
  ...
  useEffect(() => {
    const fetchTop5 = async () => {
      setLoadingTop5(true);
      const data = await getTopValue(period, 5, warehouseId);
      setTop5(data);
  ...
  const fetchTop20 = async () => {
    ...
    const data = await getTopValue(period, 20, warehouseId);
    setTop20(data);
    ...
    return (
      <Card className="border-none shadow-sm bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl h-full">
        <CardContent className="p-5 flex flex-col items-center justify-center min-h-[200px] text-center">
          ...
          <p className="font-bold text-slate-500">Belum Ada Data Nilai Bersih</p>
          <p className="text-xs text-slate-400">Tidak ada barang yang memiliki nilai di periode ini.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={handleDialogOpen}>
      <Card className="border border-emerald-100 dark:border-emerald-900/50 shadow-md bg-gradient-to-br from-emerald-50/50 to-white dark:from-emerald-950/20 dark:to-slate-900 h-full relative overflow-hidden group">
        ...
        <CardContent className="p-5 flex flex-col h-full relative z-10">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-bold text-slate-900 dark:text-white text-sm">Top 5 Nilai Tertinggi</h3>
            </div>
            <DialogTrigger asChild>
              <Button variant="ghost" size="sm" className="h-8 text-xs gap-1 text-emerald-600 hover:text-emerald-700 hover:bg-emerald-100/50">
                Lihat Top 20
            ...
            {top5.map((item, index) => (
              <div key={item.id} className="flex items-center justify-between gap-2 bg-white/60 dark:bg-slate-800/50 p-2 rounded-xl">
                ...
                <div className="flex-shrink-0 text-right">
                  <p className="text-sm font-black text-emerald-600">Rp {new Intl.NumberFormat('id-ID').format(item.inventoryValue)}</p>
                  <p className="text-[9px] text-slate-400 uppercase">Stok: {item.stockAkhir} {item.unit}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="flex items-center gap-2 text-xl">
                ...
                Top 20 Nilai Bersih Tertinggi
              </DialogTitle>
              <DialogDescription>
                Daftar barang dengan nilai investasi/stok (Inventory Value) paling tinggi pada periode yang dipilih.
              </DialogDescription>
            </div>
            <Button 
              variant="outline" 
                ...
                top20.map((item, idx) => (
                  <TableRow key={item.id} className="hover:bg-slate-50/50">
                    <TableCell className="text-center">
                      <Badge variant={idx < 3 ? "default" : "secondary"} className={idx < 3 ? "bg-emerald-500" : ""}>
                        #{idx + 1}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <span className="font-medium text-slate-600">{new Intl.NumberFormat('id-ID').format(item.stockAkhir)} {item.unit}</span>
                    </TableCell>
                    <TableCell className="text-right">
                      <div className="inline-flex items-center gap-2">
                        <span className="font-black text-emerald-600">Rp {new Intl.NumberFormat('id-ID').format(item.inventoryValue)}</span>
                      </div>
                    </TableCell>
                  </TableRow>
                ))
