...
import { SalesOrder, SpkApiPayload, SpkFormSchema, SpkFormValues } from "@/schemas/index";
...
export default function CreateFormSPKByIdSO({
    salesOrders,
    teamData,
    isLoading,
    role,
    ...
    const form = useForm<SpkFormValues>({
        ...
        defaultValues: {
            spkDate: new Date(),
            salesOrderId: preSelectedSalesOrderId || "", // Gunakan preSelectedSalesOrderId di default values
            teamId: "",
            notes: "",
            details: [],
            createdById: "",
    ...
    // Effect untuk menangani preSelectedSalesOrderId saat komponen pertama kali dimuat
    useEffect(() => {
        if (preSelectedSalesOrderId && salesOrders.length > 0) {
            const so = salesOrders.find((item) => item.id === preSelectedSalesOrderId);
            if (so) {
                setSelectedSalesOrderId(preSelectedSalesOrderId);
                setSelectedSalesOrder(so);
                form.setValue("salesOrderId", preSelectedSalesOrderId);

                // Set notes berdasarkan SO yang dipilih
    ...
    }, [preSelectedSalesOrderId, salesOrders, form]);

    const handleSalesOrderChange = (value: string) => {
        setSelectedSalesOrderId(value); // Perbaiki: gunakan value, bukan salesOrderId yang tidak terdefinisi
    ...
    const handleSubmit = async (data: SpkFormValues) => {
        try {
            ...
            ;((...args: any[]) => {})("Payload dikirim:", payload);
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <CardDescription className="text-xs md:text-base">
                            ...
                            {preSelectedSalesOrderId && (
                                <span className="text-xs md:text-base text-green-600 font-medium ml-2">
                                    (Pre-selected SO: {selectedSalesOrder?.soNumber || preSelectedSalesOrderId})
                                </span>
                            )}
                        </CardDescription>
                    </div>
                                        ...
                                        <SelectContent>
                                            {salesOrders.map((so) => (
                                                <SelectItem key={so.id} value={so.id}>
                                                    {/* Mobile: hanya tampil di layar kecil */}
