...
interface CreateButtonSPKProps {
  role?: string;
...
const CreateButtonSPK = ({
  ...
  const getBasePath = (userRole?: string) => {
    ...
    return paths[userRole ?? "admin"] || "/admin-area/logistic/spk";
  };
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.error("Error navigating to SPK create page:", error);
      setIsLoading(false);
      setIsNavigating(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="flex items-center gap-2 relative z-10">
          {withIcon && (
            <div className={`p-1 rounded-lg ${variant === "outline" ? "bg-cyan-100 text-cyan-700" : colors.iconBg} transition-all duration-200 group-hover:scale-110`}>
              <Plus className="h-4 w-4" />
            </div>
          )}
          <span className="font-medium">Tambah SPK</span>
        </div>
      )}
...
export default CreateButtonSPK;