...
const FormMonitoringProgressSpkByIDAdmin = ({ dataSpk, isLoading, role, userId }: FormMonitoringProgressSpkProps) => {
    ...
    const mapToSPKData = (raw: SPKDataApi | null): SPKData[] => {
        ;((...args: any[]) => {})('🚀 ==== mapToSPKData CALLED ====');
        ;((...args: any[]) => {})('📦 Raw data:', raw);
        ...
        ;((...args: any[]) => {})('✅ SPK Number:', raw.spkNumber);
        ;((...args: any[]) => {})('📄 spkFieldReport:', raw.spkFieldReport);
        ...
        let items = raw.salesOrder?.items?.map(itemSales => {
            ...
            ;((...args: any[]) => {})('🔍 Item:', itemSales.name);
            ;((...args: any[]) => {})('📄 All spkFieldReport:', raw.spkFieldReport);
            ...
            ;((...args: any[]) => {})('📊 Field reports for this item:', itemFieldReports);
            ...
            ;((...args: any[]) => {})('✅ Latest report:', latestReport);
            ;((...args: any[]) => {})('📊 Item Progress:', itemProgress);
    ...
    const fetchReports = useCallback(async () => {
        ...
        try {
            ...
            // Simulate loading untuk menunjukkan proses filtering
            await new Promise(resolve => setTimeout(resolve, 500));

            if (dataSpk?.spkNumber) {
                reports = reports.filter(r => r.spkNumber === dataSpk.spkNumber);
            }
    ...
    }, [filters, dataSpk?.spkNumber]);

    // ✅ HANYA 2 EFFECT YANG PENTING
    useEffect(() => {
        if (dataSpk) {
            ;((...args: any[]) => {})("DATA SPK From PAGE", dataSpk);
    ...
    }, [filters.date, filters.status, filters.itemId, dataSpk?.spkNumber, fetchReports]);
    // ✅ Sekarang fetchReports sudah termasuk dalam dependencies
          ...
          transition-all duration-200 active:scale-95"
                                    ...
                                    <div className="mt-4 flex flex-col sm:flex-row gap-2 justify-end bg-cyan-800 py-2 px-3 rounded-b-lg border-t border-border/40">
                                        {/* <Button
                                            ...
                                            size="sm"
                                            onClick={handleBack}
                                            className="hidden sm:flex text-white hover:bg-white/20 transition-colors bg-transparent h-7"
                                        >
                                        ...
                                        <CardContent>
                                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 md:gap-3">
                                                {selectedReport.photos.map((photo, idx) => {
                                                    ...
                                                    // Get coordinates if available
                                                    const photoDetail = (selectedReport as any).photoDetails?.[idx];
                                                    const hasCoordinates = photoDetail?.latitude && photoDetail?.longitude;
                                                    const mapUrl = hasCoordinates 
                                                        ? `https://maps.google.com/?q=${photoDetail.latitude},${photoDetail.longitude}`
                                                        : null;
            ...
            />
            {/* Style Animasi */}
            <style jsx>{`@keyframes fadeIn {from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); }}
                .animate-fade-in {
