...
export const ExpandableRow = forwardRef<HTMLTableRowElement, ExpandableRowProps>(
    (
        ...
        // 🟢 Financial Calculations (moved to component scope for accessibility across cells)
        const calculatePoAmount = (po: any) => {
            ...
            if (po.lines && po.lines.length > 0) {
                return po.lines.reduce((lSum: number, line: any) => {
                    ...
                    return upActual !== 0 ? lSum + (upActual * qActual) : lSum + (upEstimate * qEstimate);
                }, 0);
            }
        ...
        const childPoTotal = pr.childPrs?.reduce((sum, child) => {
            const poSum = child.purchaseOrders?.reduce((s, po) => s + calculatePoAmount(po), 0) ?? 0;
            return sum + poSum;
        }, 0) ?? 0;
                ...
                >
                    ...
                    <TableCell className="font-medium max-w-[300px]">
                        <div className="flex items-center gap-0.5 flex-wrap">
                            {/* Nomor PR */}
                            <Badge
                                className="bg-gradient-to-r from-blue-500 to-blue-600 text-white border-0 text-xs px-1.5 py-0.5 cursor-pointer hover:from-blue-600 hover:to-blue-700 transition-all"
                                onClick={(e) => {
                                    e.stopPropagation();
                                    if (onPrClick) onPrClick(pr);
                                }}
                            >
                            ...
                            {!pr.spkId ? (
                                ...
                                <Fragment>
                                    ...
                                    {pr.childPrs && pr.childPrs.length > 0 && (
                                        pr.childPrs.map((child) => (
                                            <Link
                                                key={child.id}
                                                href={`${role === 'pic' ? '/pic-area' : role === 'super' ? '/super-admin-area' : '/admin-area'}/logistic/pr?search=${encodeURIComponent(child.nomorPr)}&tab=project&page=1`}
                                                className="group"
                                                onClick={(e) => e.stopPropagation()}
                                            >
                                                <Badge
                                ...
                                <Fragment>
                                    ...
                                    {pr.parentPr && (
                                        <Link
                                            href={`${role === 'pic' ? '/pic-area' : role === 'super' ? '/super-admin-area' : '/admin-area'}/logistic/pr?search=${encodeURIComponent(pr.parentPr.nomorPr)}&tab=umum&page=1`}
                                            className="group"
                                            onClick={(e) => e.stopPropagation()}
                                        >
                                            <Badge
                    ...
                    <TableCell className="text-right">
                        <div className="flex flex-col items-end gap-1.5">
                            <div className="bg-gradient-to-r from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 border border-blue-200 dark:border-blue-800 rounded-lg px-3 py-1.5 text-right w-full">
                                <div className="flex items-center justify-end gap-2">
                                    <BanknoteArrowUp className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />
                                    <span className="text-sm font-bold text-gray-900 dark:text-white">
                                        {formatCurrency(pr.uangMuka?.[0]?.jumlah ?? 0)}
                                    </span>
                                </div>
                                <div className="text-[10px] text-blue-700 dark:text-blue-300 font-bold uppercase mt-0.5">ACC FINANCE</div>
                            </div>
                            <PercentageBadge
                    ...
                    <TableCell>
                        <div className="flex flex-col gap-1.5">
                            <div className="flex items-center gap-2">
                                ...
                                <Badge
                                    variant="outline"
                                    className={cn(STATUS_CONFIG[pr.status].color, "border font-medium text-[11px] px-2 py-0.5 whitespace-nowrap")}
                                >
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Badge
                                    variant="outline"
                                    className={cn(STATUS_APPROVE_CONFIG[approveStatus].color, "border font-medium text-[11px] px-2 py-0.5 whitespace-nowrap")}
                                >
