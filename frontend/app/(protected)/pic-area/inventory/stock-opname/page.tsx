...
export default function StockOpnamePage() {
    ...
    // 🔐 Guard role
    useEffect(() => {
        if (!sessionLoading && user?.role !== "pic" && user?.role !== "admin" && user?.role !== "inventory_manager" && user?.role !== "super") {
    ...
    // Load initial data
    useEffect(() => {
        if (user?.role === "pic" || user?.role === "admin" || user?.role === "inventory_manager" || user?.role === "super") {
            fetchStockOpnames();
            fetchWarehouses();
        }
    }, [filters, user?.role]);
    ...
    const handleViewDetail = (id: string) => {
        router.push(`/pic-area/inventory/stock-opname/${id}`);
    };

    const handleEdit = (id: string) => {
        router.push(`/pic-area/inventory/stock-opname/${id}/edit`);
    };
                ...
                <div className="space-y-4 p-2 pt-1 md:p-4">
                    ...
                    <Breadcrumb>
                        <BreadcrumbList>
                            ...
                            <BreadcrumbSeparator />
                            <BreadcrumbItem>
                                <BreadcrumbLink href="/pic-area/inventory/dashboard">
                                    Inventory
                                </BreadcrumbLink>
                            </BreadcrumbItem>
                            <BreadcrumbSeparator />
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-6">
                        ...
                        {(user?.role === "pic" || user?.role === "admin" || user?.role === "super") && (
                            <div className="flex items-center gap-2 w-full md:w-auto">
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className="w-56 p-2">
                                        ...
                                        <DropdownMenuItem
                                            onClick={() => router.push("/pic-area/inventory/stock-opname/create?type=PERIODIC")}
                                        ...
                                        <DropdownMenuItem
                                            onClick={() => router.push("/pic-area/inventory/stock-opname/create?type=AD_HOC")}
                                            className="flex flex-col items-start gap-1 p-3 cursor-pointer"
                                        >
                                        ...
                                        <DropdownMenuItem
                                            onClick={() => router.push("/pic-area/inventory/stock-opname/create?type=INITIAL")}
                                            className="flex flex-col items-start gap-1 p-3 cursor-pointer"
                                        >
